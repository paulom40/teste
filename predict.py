...
# Create tabs - THIS IS THE CRITICAL LINE THAT WAS MISSING
...
with tab5:
    ...
    if sort_option == "Team Name":
        ...
        display_df['Over 2.5 %_num'] = display_df['Over 2.5 %'].str.rstrip('%').astype(float)
        display_df = display_df.sort_values('Over 2.5 %_num', ascending=False)
        display_df = display_df.drop('Over 2.5 %_num', axis=1)
    ...
    columns_to_show = ['Team', 'Matches', 'Avg Total Goals', 'Over 2.5 %', 'Over 3.5 %']
    
    if 'HST' in df.columns:
        columns_to_show.extend(['Avg Total SOT', 'Over 10.5 SOT %'])
    
    if 'HC' in df.columns:
        columns_to_show.extend(['Avg Total Corners', 'Over 10.5 Corners %'])
    ...
    with col1:
        ...
        if df['TotalCorners'].sum() > 0:
            ...
            st.dataframe(corners_market_df.style.
