...
sport = st.sidebar.radio(
    "Choose Sport",
    ["Soccer", "Tennis", "Basketball", "NBA", "Horse Racing"]
)
...
if "bets" not in st.session_state:
    st.session_state.bets = pd.DataFrame(
        columns=[
            "Date", "Sport", "Tipster", "Event", "Selection",
            "Odds", "Stake", "Result", "P/L"
        ]
    )
...
# DAILY TIP GENERATOR
# -------------------------------------------------
def generate_daily_tips(sport: str, tipster_name: str, num_tips: int = 4):
    tmpl = TIPSTER_TEMPLATES.get(sport, {}).get(tipster_name, {})
    if not tmpl:
        return []
    ...
    for _ in range(num_tips):
        if sport == "Tennis":
            p1 = random.choice(tmpl["players"])
            p2 = random.choice(tmpl["opponents"])
        else:
            p1 = random.choice(tmpl["teams"])
            p2 = random.choice(tmpl["opponents"])
        while p2 == p1:
            p2 = random.choice(tmpl["opponents"])

        sel = random.choice(tmpl["markets"])
        if "Player A" in sel:
            sel = sel.replace("Player A", p1).replace("Player B", p2)
        elif "Player B" in sel:
            sel = sel.replace("Player B", p1).replace("Player A", p2)

        odds = round(random.uniform(*tmpl["odds_range"]), 2)
        reason = (
            f"{p1} strong at home; {p2} leaky defense."
            if "Over" in sel
            else f"{p1} form; value at odds."
        )

        tips.append({
            "Date": today,
            "Match": f"{p1} vs {p2} (ATP)" if sport == "Tennis"
                     else f"{p1} vs {p2} (NBA)" if sport == "NBA"
                     else f"{p1} vs {p2} (PL)",
            "Selection": sel,
            "Odds": odds,
            "Outcome": "Pending",
...
# -------------------------------------------------
# AUTO‑GENERATE TIPS ON FIRST LOAD
# -------------------------------------------------
...
def auto_generate():
    ...
    for s in TIPSTER_TEMPLATES:
        ...
        for tipster in TIPSTER_TEMPLATES[s]:
            ...
            if df.empty or df[df["Date"] == today].empty:
                new = generate_daily_tips(s, tipster, 4)
                new_df = pd.DataFrame(new)
                df = pd.concat([df, new_df], ignore_index=True)
                st.session_state.tipsters_data[s][tipster]["tips"] = df.to_dict("list")

auto_generate()   # <-- runs on every fresh session
    ...
    if st.button("Update Tips & Injuries", type="primary"):
        with st.spinner("Refreshing..."):
            ...
            if random.random() < 0.4:
                ...
                st.session_state.injury_data = pd.concat([st.session_state.injury_data, new], ignore_index=True)
...
with tab2:
    ...
    filtered = st.session_state.injury_data[
        st.session_state.injury_data["Impact"].isin(impact)
    ]

    def color_impact(val):
        return (
            "background-color: #ffcccc" if val == "High"
            else "background-color: #fff3cd" if val == "Med"
            else ""
        )

    styled = filtered.style.map(color_impact, subset=["Impact"])
    st.dataframe(styled, width="stretch")   # FIXED
...
c2.metric(
    "Total P/L",
    f"£{st.session_state.bets['P/L'].sum():+.2f}"
    if not st.session_state.bets.empty else "£0.00"
)
roi = (
    st.session_state.bets["P/L"].sum() / st.session_state.bets["Stake"].sum() * 100
    if not st.session_state.bets.empty and st.session_state.bets["Stake"].sum() > 0
    else 0
)
...
# -------------------------------------------------
st.markdown("---")
st.caption("**Auto‑generates tips on load** | No `applymap` / `use_container_width` warnings | 18+")
