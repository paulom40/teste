...
def calculate_enhanced_elo(df, k=32, surface_k=35, window=20):
    """
    Enhanced ELO calculation with surface-weighted ratings.
    ELO and WELO now incorporate surface performance, giving more weight to the current surface.
    """
    players = set(df['winner'].dropna().unique()) | set(df['loser'].dropna().unique())

    # Base ELO (surface-weighted average)
    ...
    # Surface-weighted ELO (combines all surfaces with current surface having more weight)
    surface_weighted_elo = {p: {'Hard':1500.0, 'Clay':1500.0, 'Grass':1500.0} for p in players}
    ...
    for _, row in df_sorted.iterrows():
        ...
        # Calculate recent form (last N matches overall)
        ...
        # ====================================================================
        # UPDATE SURFACE-SPECIFIC ELO FIRST (most important)
        # ====================================================================
        s1 = surface_elo[w][surf]
        s2 = surface_elo[l][surf]
        
        # Add form bonus to surface ELO
        s1_adjusted = s1 + 60 * (w_surf_form - 0.5)
        s2_adjusted = s2 + 60 * (l_surf_form - 0.5)
        
        exp_s1 = 1 / (1 + 10 ** ((s2_adjusted - s1_adjusted) / 400))

        surface_elo[w][surf] += surface_k * (1 - exp_s1)
        surface_elo[l][surf] += surface_k * (0 - (1 - exp_s1))

        # ====================================================================
        # UPDATE SURFACE-WEIGHTED ELO (70% current surface, 30% others)
        # ====================================================================
        # This creates a composite ELO that heavily weights the current surface
        for player in [w, l]:
            current_surf_elo = surface_elo[player][surf]
            other_surfaces = [s for s in ['Hard', 'Clay', 'Grass'] if s != surf]
            other_avg = sum(surface_elo[player][s] for s in other_surfaces) / 2
            
            # 70% current surface, 30% average of other surfaces
            surface_weighted_elo[player][surf] = 0.70 * current_surf_elo + 0.30 * other_avg

        # ====================================================================
        # UPDATE GENERAL ELO (based on surface-weighted performance)
        # ====================================================================
        # Use surface-weighted ELO for calculation
        r1 = surface_weighted_elo[w][surf] + 50 * (w_form - 0.5)
        r2 = surface_weighted_elo[l][surf] + 50 * (l_form - 0.5)

        exp1 = 1 / (1 + 10 ** ((r2 - r1) / 400))

        # Update general ELO with smaller K-factor (surface ELO is primary)
        elo[w] += (k * 0.8) * (1 - exp1)
        elo[l] += (k * 0.8) * (0 - (1 - exp1))

        # ====================================================================
        # UPDATE WEIGHTED ELO (recent performance weighted average)
        # ====================================================================
        # WELO gives even more weight to recent matches on current surface
        welo[w] = welo[w] * 0.80 + surface_weighted_elo[w][surf] * 0.20
        welo[l] = welo[l] * 0.80 + surface_weighted_elo[l][surf] * 0.20
...
def build_features(p1, p2, surface, player_stats, h2h, h2h_surface, match=None):
    ...
    # Surface-specific H2H (MORE IMPORTANT)
    ...
    feat = [
        # MOST IMPORTANT: Surface-specific ELO (3x weight via position)
        s1['surface_elo'][surf] / (s2['surface_elo'][surf] + 1),
        s1['surface_elo'][surf] / (s2['surface_elo'][surf] + 1),  # Duplicate for emphasis
        s1['surface_elo'][surf] / (s2['surface_elo'][surf] + 1),  # Triplicate for 3x weight
        
        # Secondary: Weighted ELO (surface-aware)
        s1['welo'] / (s2['welo'] + 1),
        s1['welo'] / (s2['welo'] + 1),  # Duplicate for 2x weight
        
        # Tertiary: General ELO (least important)
        s1['elo'] / (s2['elo'] + 1),

        # Surface-specific win rates (VERY IMPORTANT)
        s1['surface_win_rate'][surf] - s2['surface_win_rate'][surf],
        (s1['surface_win_rate'][surf] - s2['surface_win_rate'][surf]) * 2,  # 2x weight
        
        # Recent form and momentum
        s1['recent_form'] - s2['recent_form'],
        s1['very_recent_form'] - s2['very_recent_form'],
        
        # General win rate (less important than surface-specific)
        s1['win_rate'] - s2['win_rate'],

        # Rankings (inverse ratio - lower rank is better)
        (s2['avg_rank'] + 1) / (s1['avg_rank'] + 1),

        # H2H features (surface-specific MORE important)
        h2h_surf_ratio,  # Surface H2H
        h2h_surf_ratio,  # Duplicate for 2x weight
        h2h_ratio,       # General H2H

        # Game length prediction
        (s1['avg_games'] + s2['avg_games']) / 2,

        # Experience (surface experience MORE important)
        surf_exp_ratio,
        surf_exp_ratio,  # Duplicate for 2x weight  
...
def train_models(df, player_stats, h2h, h2h_surface):
    ...
    # Enhanced model parameters - optimized for surface-based features
    model_winner = GradientBoostingClassifier(
        n_estimators=300,           # More trees for complex surface patterns
        max_depth=6,                # Deeper trees to capture surface interactions
        learning_rate=0.03,         # Slower learning for stability
        random_state=42,
        subsample=0.8,              # Prevent overfitting
        min_samples_split=15,       # Regularization
        min_samples_leaf=8,         # Regularization
    ...
    model_ou = GradientBoostingClassifier(
        n_estimators=250, max_depth=5, learning_rate=0.03, random_state=42,
...
def main():
    st.title("🎾 ATP & Challenger Tennis Predictor - SURFACE-BASED ELO")
    st.markdown("**Surface-Weighted ELO + Surface-Specific H2H + Enhanced Feature Engineering**")
    st.markdown("*ELO and WELO now calculated based on surface performance - clay specialists get higher ratings on clay!*")
