    ...
    if "matches" in st.session_state and st.session_state.matches:
        ...
        for idx, m in enumerate(st.session_state.matches):
            with cols[col_idx % 2]:
                ...
                with col1:
                    o1 = float(st.number_input("P1 Odd", 1.01, 10.0, 1.8, step=0.01, key=f"p1_{idx}"))
                with col2:
                    o2 = float(st.number_input("P2 Odd", 1.01, 10.0, 2.0, step=0.01, key=f"p2_{idx}"))
                with col3:
                    o_under = float(st.number_input("U21.5 Odd", 1.01, 5.0, 2.0, step=0.01, key=f"ou_{idx}"))
                with col4:
                    o_over = float(st.number_input("O21.5 Odd", 1.01, 5.0, 1.7, step=0.01, key=f"oo_{idx}"))

                # Predictions with ACTUAL odds entered by user
