# Leagues.py - FOOTBALL PREDICTOR PRO v2.1 (NO FPDF – 100% COMPATIBLE)
...
**Realistic Match Predictions – xG, Shots, Win %, Form, PDF Export**
""")
...
if df['DATE'].isna().all():
    st.error("Date parse failed. Use DD/MM/YYYY.")
    st.stop()
...
def compute_form_stats(df: pd.DataFrame, last_n: int = 5) -> dict:
    ...
    for team in df['HOMETEAM'].unique():
        m = df[df['HOMETEAM'] == team].sort_values('DATE').tail(last_n)
        if len(m) > 0:
            home_games.append({
                'team': team,
    ...
    for team in df['AWAYTEAM'].unique():
        m = df[df['AWAYTEAM'] == team].sort_values('DATE').tail(last_n)
        if len(m) > 0:
            away_games.append({
                'team': team,
...
def predict_match(home: str, away: str):
    ...
    home_shots = max(3.0, min(round(base_home_shots * ha * (2 - aa) / 2, 1), 7.5))
    away_shots = max(2.0, min(round(base_away_shots * aa * (2 - hd) / 2, 1), 6.5))
    ...
    home_corners = max(2.0, min(round(stats['home_corners'].get(home, 6.0) * ha * (2 - aa) / 2, 1), 10.0))
    away_corners = max(1.0, min(round(stats['away_corners'].get(away, 4.5) * aa * (2 - hd) / 2, 1), 8.0))
...
def plot_form(team, is_home=True):
    if is_home:
        m = df[df['HOMETEAM'] == team].tail(5)
        gf, ga = 'FTHG', 'FTAG'
    else:
        m = df[df['AWAYTEAM'] == team].tail(5)
        gf, ga = 'FTAG', 'FTHG'
    if m.empty:
        return go.Figure().add_annotation(text="No games", x=0.5, y=0.5, showarrow=False)
    fig = go.Figure()
...
html_content = f"""
<!DOCTYPE html>
<html><head><title>{home_team} vs {away_team}</title>
<style>
...
<div class="metric"><strong>Predicted Score:</strong> {result['home_goals']} – {result['away_goals']}</div>
<div class="metric"><strong>xG:</strong> {result['home_xg']} – {result['away_xg']}</div>
<div class="metric"><strong>Shots:</strong> {result['home_shots']} – {result['away_shots']}</div>
<div class="metric"><strong>Corners:</strong> {result['home_corners']} – {result['away_corners']}</div>
<div class="metric"><strong>Win Probability:</strong> {home_team} {result['home_win']}%, Draw {result['draw']}%, {away_team} {result['away_win']}%</div>
</body></html>
"""
...
if st.button("Export to PDF"):
    ...
    st.info("Click above → Open in browser → Print → Save as PDF")
