...
def main():
    ...
    # File upload FIRST - more prominent
    st.sidebar.markdown("### 📥 Option 1: Upload Your Excel File")
    uploaded_file = st.sidebar.file_uploader(
        "Select an Excel file (.xlsx or .xls)",
        type=['xlsx', 'xls'],
        help="Upload your WTA match data file with columns: Winner, Loser, W1-W5, L1-L5, WRank, LRank, Surface, Date, Wsets"
    )
    ...
    if uploaded_file is not None:
        ...
        # If no file uploaded, offer GitHub option
        st.sidebar.markdown("---")
        st.sidebar.markdown("### 🌐 Option 2: Use GitHub Data")
        
        if st.sidebar.button("📥 Load GitHub WTA Database", use_container_width=True):
            ...
            st.sidebar.info("ℹ️ Upload an Excel file above OR click to load GitHub data")
            return
