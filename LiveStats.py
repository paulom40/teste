# app.py — Football Predictor Pro v11.2 ELITE (2025 Sharp Model)
# Shots Inside Box + Big Chances → the most accurate public halftime model
...
# ============================= PAGE CONFIG =============================
st.set_page_config(page_title="Predictor Pro v11.2 ELITE", layout="wide", page_icon="target")

st.markdown("""
<style>
    .title {font-size:62px !important; font-weight:bold; text-align:center; color:#FF006E; margin-bottom:0px;}
    .tag {font-size:24px; text-align:center; color:#00FFAA; margin-top:-10px;}
</style>
<div class="title">PREDICTOR PRO v11.2</div>
<div class="tag">Shots Inside Box + Big Chances Model • 2025 Sharp Edge</div>
""", unsafe_allow_html=True)

# ============================= 2025/26 LEAGUE AVERAGES (real sharp numbers) =============================
def get_leagues():
    return {
        ...
        'Serie B (ITA)':        {'avg_goals':2.42, 'home_adv':1.33, '2h_ratio':0.510, 'vol':0.79, 'avg_sib':7.1,  'big_chance':1.8, 'tier':2},
        'La Liga 2 (ESP)':      {'avg_goals':2.31, 'home_adv':1.34, '2h_ratio':0.505, 'vol':0.77, 'avg_sib':6.9,  'big_chance':1.7, 'tier':2},
    }
...
def main():
    ...
    with col1:
        ...
        home_team = st.text_input("Team", "Arsenal")
        h_goals = st.number_input("1H Goals", 0, 10, 1)
        h_xg = st.number_input("1H xG", 0.0, 10.0, 1.6, 0.1)
        h_sib = st.number_input("Shots Inside Box", 0, 25, 7)
        h_big = st.number_input("Big Chances Created", 0, 10, 2)
        h_poss = st.slider("Possession %", 0, 100, 56)
    ...
    with col2:
        ...
        a_big = st.number_input("Big Chances Created ", 0, 10, 1)
        a_poss = st.slider("Possession % ", 0, 100, 44)
    ...
    with col2:
        ...
        if result['total_xg'] > 2.4:    st.success(f"OVER 2.5 2H GOALS — EDGE")
